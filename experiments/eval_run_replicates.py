...
## Reproducibility:
## ################
# Do not fix seed to get random weight initialisation and random sampling

## Data preparation
## ################

#x_sequences = ["A", "G"]
#x_sequences = ["AAG", "AGA", "GAA"]

# Evolve sequences
x_fasta_file = output_path+"/train.fasta" # training sequences
v_fasta_file = output_path+"/valid.fasta" # validation sequences

if os.path.isfile(x_fasta_file) and os.path.isfile(v_fasta_file) and from_fasta:
...
def eval_evomodel(EvoModel, m_args, in_args):
    ...
    #N = in_args["X_counts"].sum().item()
    #[values.__imul__(1/N) for values in return_list ]

    return_list = np.array(return_list) #/N
    return return_list
...
if os.path.isfile(scores_file) and scores_from_file:
    ...
    new_scores = []
    for score in scores:
        new_scores.append(score[:,:min_iter])

    new_scores = np.array(new_scores)
    dump(new_scores, scores_file)
...
if verbose: print("\nPlotting..")
plt_elbo_ll_kl_rep_figure(new_scores, output_path+"/rep_fig",
