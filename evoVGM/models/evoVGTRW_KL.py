...
from evoVGM.models import NetXGTRProbDecoder

from evoVGM.utils import timeSince
from evoVGM.models import BaseEvoVGM_KL
...
class EvoVGTRW_KL(nn.Module, BaseEvoVGM_KL):
    def __init__(self, 
                 x_dim,
                 a_dim,
                 h_dim,
        ...
        self.gtrFreqEncoder = GTRfreqIndDeepDirEncoder(
                ...
                device=self.device)
        # decoder
        self.decoder = NetXGTRProbDecoder(
                self.a_dim, 
                self.h_dim,
                n_layers=self.x_layers,
                device=self.device)
...
#         kl_abrf_ws = N * (b_kl_ws + r_kl_ws + f_kl_ws)/self.m_dim
        kl_abrf_ws = b_kl_ws + r_kl_ws + f_kl_ws
#         kl_abrf_ws = b_kl_ws.sum() + r_kl_ws + f_kl_ws
#         print("kl_abrf_ws")
#         print(kl_abrf_ws.shape) # [m_dim, 1]
#         print(kl_abrf_ws)
...
#             a_n, a_kl_n = self.ancestEncoder(x_n, latent_sample_size)
            a_n, a_kl_n = self.ancestEncoder(x_n, latent_sample_size, a_sample_temp=0.1)
#             a_n, a_kl_n = self.ancestEncoder(latent_sample_size)
            ...
            # ############################
            x_recons_n, loglx_n = self.decoder(a_n, x_n_expanded, tm)
...
#         print(kl_abrf_ws)
        ...
        # Compute ELBO
        ########################
#         kl_abrf_ws += a_kl_ws.expand([self.m_dim, 1])/self.m_dim
        kl_abrf_ws += a_kl_ws.expand([self.m_dim, 1]) #/self.m_dim
